...
class Cron_Command extends EE_Command {
	...
	public function __construct() {
		if ( 'running' !== EE_DOCKER::container_status( 'ee-cron-scheduler' ) ) {
			$cron_scheduler_run_command = 'docker run --name ee-cron-scheduler --restart=always -d -v ' . EE_ROOT_DIR . '/cron:/etc/ofelia:ro -v /var/run/docker.sock:/var/run/docker.sock:ro easyengine/cron:v' . EE_VERSION;
	...
	private function update_cron_config() {
		...
		file_put_contents( EE_ROOT_DIR . '/cron/config.ini', $config );
